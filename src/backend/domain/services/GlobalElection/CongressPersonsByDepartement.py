class CongressPersonsByDepartments : 
    ...
    def regroup_from_congress_dpts(self, departments_congress):
        congress_persons_by_dpts = {}
        for department_congress in departments_congress : 
            dpt_code = str(department_congress.department_code)
            congress_persons = self.__search_congress_persons_for_this_dpt(departments_congress, dpt_code)
            congress_persons_sorted = sorted(congress_persons, key=lambda x: x.last_name)
            congress_persons_by_dpts[dpt_code] = congress_persons_sorted
